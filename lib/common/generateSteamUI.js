...
export async function generateSteamNotification(groupId, notifications, gameDetailsCache = {}) {
  const screenshotTasks = notifications.map(async (notification) => {
    try {
      ...
      let gamelogoUrl = finalTemplateData.gamelogo || '';
      if (gameDetails?.assets?.header) {
        if (gameDetails.assets.asset_url_format) {
          // 优先使用模板格式化URL
          gamelogoUrl = `https://cdn.akamai.steamstatic.com/${gameDetails.assets.asset_url_format.replace('${FILENAME}', gameDetails.assets.header)}`;
        } else {
          // 后备方案：直接拼接
          gamelogoUrl = `https://cdn.akamai.steamstatic.com/steam/apps/${gameDetails.id}/${gameDetails.assets.header}`;
        }
      }
...
export async function generateInventoryUpdateImage(steamId, newGamesDetails) {
  try {
    ...
    for (const game of newGamesDetails) {
      ...
      let headerUrl = game.header_image || '';
      if (game.assets?.asset_url_format && game.assets?.header) {
        const path = game.assets.asset_url_format.replace('${FILENAME}', game.assets.header);
        headerUrl = `https://cdn.akamai.steamstatic.com/${path}`;
      } else if (game.assets?.header) {
        headerUrl = `https://cdn.akamai.steamstatic.com/steam/apps/${appId}/${game.assets.header}`;
      }
      ...
      } else if (game.price_overview) { // 保留旧字段作为后备
        adaptedPriceOverview = {
