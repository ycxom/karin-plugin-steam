...
import { getSteamIDFromFriendCode, convertFriendCodeToSteamID64 } from './scrapeSteam.js';
...
export async function serveBase64ImageForHelp() {
  ...
  try {
    const imagePath = path.resolve(`${Config.dirPath}/resources/img/help_image.png`);
    if (!fs.existsSync(imagePath)) {
      throw new Error('Help image not found');
    }
    ...
    const screenshotBuffer = fs.readFileSync(imagePath);
    const base64Image = screenshotBuffer.toString('base64');
