...
export function setGroupGameOnlyMode(groupId, enabled) {
    const sql = 'UPDATE steam_group_settings SET game_only_mode = ? WHERE group_id = ?';
    return new Promise((resolve, reject) => {
        db.run(sql, [enabled ? 1 : 0, String(groupId)], function (err) {
            if (err) return reject(err);
            // 如果没有记录被更新，则插入新记录
            if (this.changes === 0) {
                const insertSql = 'INSERT INTO steam_group_settings (group_id, enabled, game_only_mode) VALUES (?, 0, ?)';
                db.run(insertSql, [String(groupId), enabled ? 1 : 0], function (insertErr) {
...
export function getGroupGameOnlyMode(groupId) {
    const sql = 'SELECT game_only_mode FROM steam_group_settings WHERE group_id = ?';
    return new Promise((resolve, reject) => {
        db.get(sql, [String(groupId)], (err, row) => {
            if (err) return reject(err);
            resolve(row ? Boolean(row.game_only_mode) : false);
        });
    });
}
