...
import { getGameLogoAsBase64 } from '../common/getGameLogo.js';
...
export async function renderLibraryImage(steamUserInfo) {
    try {
        const steamId = typeof steamUserInfo === 'object' && steamUserInfo !== null ? steamUserInfo.steamid : steamUserInfo;

        if (!steamId) {
            throw new Error("无效的Steam用户信息，未能找到 steamid。");
        }

        const [userProfile, games] = await Promise.all([
            fetchPlayerProfileAPI(steamId),
        ...
        const gameDataPromises = games.map(async (game, index) => {
            ...
            const coverBase64 = await getGameLogoAsBase64(details, game.appid);
        ...
        const [backgroundDataUri, userAvatarImg, frameImg] = await Promise.all([
            ...
            getCachedImageAsBase64(userProfile.playerAvatarImg, `avatar_${userProfile.avatarhash}`),
        ...
        logger.error(`[renderLibraryImage] 渲染库存失败 for ${steamUserInfo?.steamid || steamUserInfo}:`, error);
        throw error;
    }
}