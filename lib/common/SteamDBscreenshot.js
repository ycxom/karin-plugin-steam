...
async function handleCloudflareProtection(page) {
  try {
    ...
    logger.info('检测到 Cloudflare 验证容器');
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      logger.info(`等待 ${randomDelay1} 毫秒后模拟移动`);
      ...
      logger.info(`鼠标随机移动到 (${targetX}, ${targetY})，速度: ${randomSteps} 步`);
      await page.mouse.move(targetX, targetY, { steps: randomSteps });

      logger.info(`精确移动到点击位置 (${clickPosition.x}, ${clickPosition.y})`);
      await page.mouse.move(clickPosition.x, clickPosition.y, { steps: 5 });

      const randomDelay = Math.floor(Math.random() * 2000) + 500;
      logger.info(`等待 ${randomDelay} 毫秒后模拟点击`);
      await page.waitForTimeout(randomDelay);

      await page.mouse.click(clickPosition.x, clickPosition.y);
      logger.info(`第 ${i + 1} 次尝试点击位置 (${clickPosition.x}, ${clickPosition.y})`);
    ...
    if (!responseExists) {
      logger.warn('在规定时间内未能完成 Cloudflare 验证');
    } else {
      logger.info('Cloudflare 保护验证完成');
    }

  } catch (error) {
    logger.error('处理 Cloudflare 保护时出错:', error);
  }
}
...
export async function screenshotSteamCharts() {
  ...
  logger.info(`使用随机用户代理: ${userAgent}`);
  ...
  try {
     ...
     * 测试状态，暂时绕过cf验证
        ...
        try {
          ...
          logger.warn('Cloudflare 验证未成功，但继续进行截图:', error);
        }
     */
    ...
    logger.info('正在截取整个页面并转换为 base64');
    const screenshotBuffer = await page.screenshot({
      fullPage: true,
    });

    const base64Image = screenshotBuffer.toString('base64');
    logger.info('截图成功捕获整个页面');
    return base64Image;
