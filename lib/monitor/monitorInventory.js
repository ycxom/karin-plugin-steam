    ...
    try {
        ...
        if (previousAppIds.length === 0) {
            ...
            if (newAppIds.length > 0) {
                newGameNames = newAppIds.map(id => ({
                    appId: id,
                    name: appListCache[id] || `未知游戏(AppID: ${id})`
                }));
                const gameNamesForLog = newGameNames.map(g => g.name).join(', ');
                logger.mark(`[库存监控] 用户 ${steamId} 新增游戏: ${gameNamesForLog}`);
            }
        }
