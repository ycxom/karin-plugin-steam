...
export async function generateSteamNotification(groupId, notifications) {
    ...
    await browser.close();
    return imageB64s; // ✅ 返回一个包含所有图片Base64的数组
}
 ...
 * @param {string[]} newGameNames - 新增游戏的名称数组
 * @returns {Promise<string>} - Base64编码的图片字符串
 */
export async function generateInventoryUpdateImage(steamId, newGameNames) {
  ...
  try {
    ...
    const templateData = {
      ...
      newGames: newGameNames.map(name => ({ name })),
      gameCount: newGameNames.length,
    };
