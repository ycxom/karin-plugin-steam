    ...
    try {
      ...
      if (Config.enableAvatarFrame) {
        try {
          ...
          if (apiUserInfo && apiUserInfo.frameImg) {
            finalTemplateData.frameImg = apiUserInfo.frameImg;
          }
        } catch (apiError) {
      ...
      finalTemplateData.gamelogo = gameDetails?.assets?.header
...
export async function generateInventoryUpdateImage(steamId, newGamesDetails) {
  try {
    if (!Handlebars.helpers.contentDescriptorName) {
      Handlebars.registerHelper('contentDescriptorName', function (id) {
        const descriptors = {
          1: "成人内容",
          2: "暴力内容",
          3: "血腥内容",
    ...
    for (const appId in newGamesDetails) {
      ...
      if (game.achievements && game.achievements.highlighted) {
        await Promise.all(game.achievements.highlighted.map(async achievement => {
          if (achievement.path) {
            achievement.path = await imageUrlToBase64(achievement.path);
          }
          return achievement;
        }));
      }
      ...
      const [headerBase64, screenshotsBase64] = await Promise.all([
        ...
        Promise.all(screenshotsArray.slice(0, 4).map(ss => imageUrlToBase64(ss.path_thumbnail)))
      ]);
      ...
      const processedGame = {
        ...
        screenshots: screenshotsBase64.map((b64, index) => ({ path_thumbnail: b64 })),
    ...
    const backgroundUrlToFetch = processedGames[0]?.background_raw || '';
