...
export async function fetchSteamFreebies() {
    try {
        ...
        const freebies = items.map(item => {
            ...
            const ratingMatch = description.match(/评分: (.*?)<br>/);
            const chineseSupportMatch = description.match(/支持中文: (.*?)<br>/);
            const deadlineMatch = description.match(/截止时间: (.*?)<br>/);
            ...
            return {
                ...
                chineseSupport: chineseSupportMatch ? chineseSupportMatch[1] : '',
