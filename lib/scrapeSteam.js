...
export async function fetchSteamFriends(playerId) {
  const url = `https://steamcommunity.com/profiles/${playerId}/friends/`;
  try {
    const response = await axios.get(url, {
      timeout: 10000,
      headers: {
    ...
    $('.friend_block_v2').each((i, element) => {
      const name = $(element).find('.friend_block_content').text().trim();
      const profileLink = $(element).find('.selectable_overlay').attr('href');
      const avatar = $(element).find('.player_avatar img').attr('src');
      friends.push({ name, profileLink, avatar });
    });
