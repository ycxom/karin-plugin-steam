...
export async function makeResilientSteamApiRequest(urlBuilder) {
    ...
    for (const key of keysToTry) {
        ...
        try {
            ...
            // 请求成功，直接返回数据
            logger.debug(`使用Key...${key.slice(-4)} 成功请求 ${url}`);
            return response.data;
        } catch (error) {
            if (error.response && error.response.status === 429) {
                logger.debug(`Key...${key.slice(-4)} 已达到请求上限 (429)，自动尝试下一个Key...`);
            } else {
                logger.debug(`使用Key...${key.slice(-4)} 请求失败: ${error.message}`);
            }
        }
    }

    logger.debug(`所有 ${keysToTry.length} 个API Key均请求失败，本次操作将忽略。`);
