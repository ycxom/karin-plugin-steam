...
export async function fetchSteamStatus(playerIdentifier) {
    ...
    try {
      ...
      const steamStatus = await page.evaluate(() => {
        ...
        const playerAvatarImg = document.querySelector('.playerAvatarAutoSizeInner img[src*="avatars.akamai.steamstatic.com"]')?.src || document.querySelector('.playerAvatarAutoSizeInner img[src*="avatars.st.dl.eccdnx.com"]')?.src ||'默认头像链接';
  ...
  try {
      ...
      if (players && players.length > 0) {
          ...
          return {
              ...
              profileStatusClass: player.personastate, // 状态值: 0-离线, 1-在线, 2-忙碌, 3-离开, 4-打盹, 5-愿意交易, 6-愿意游戏
