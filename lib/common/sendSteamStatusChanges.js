...
import { getGroupGameOnlyMode } from '../db/databaseOps.js';
...
async function handleStatusChanges(changedUsers) {
  ...
  const gameDetailsCache = allGameIds.size > 0
    ? await fetchStoreItemDetails(Array.from(allGameIds), { include_assets: true })
    : {};
  ...
  for (const [groupId, changes] of changesByGroup.entries()) {
    try {
      ...
      const messagePromises = changes.map(change =>
        getStatusMessage(change.status, change.previousStatus, gameDetailsCache, gameOnlyMode)
      );
 ...
 * 根据新旧状态和新的数据结构生成通知文字
 */
async function getStatusMessage(current, previous, gameDetailsCache, gameOnlyMode = false) {
